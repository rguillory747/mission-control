...
```bash
curl -s -X POST "https://fine-lark-515.convex.site/api/heartbeat" -H "Content-Type: application/json" -d '{"name": "Ghost", "status": "active"}'
```

**End of EVERY session — LAST thing you do:**
```bash
curl -s -X POST "https://fine-lark-515.convex.site/api/heartbeat" -H "Content-Type: application/json" -d '{"name": "Ghost", "status": "sleeping"}'
```

**Log every significant action:**
```bash
curl -s -X POST "https://fine-lark-515.convex.site/api/activity" -H "Content-Type: application/json" -d '{"agent": "Ghost", "action": "DESCRIPTION"}'
```
...
```bash
curl -s -X POST "https://fine-lark-515.convex.site/api/task-create" -H "Content-Type: application/json" -d '{"title":"TASK_TITLE","description":"...","status":"inbox","assignee":"Ghost","priority":"P0"}'
```
