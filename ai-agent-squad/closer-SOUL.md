...
```bash
curl -s -X POST "https://mission-control-aiorg.vercel.app/api/heartbeat" -H "Content-Type: application/json" -d '{"name": "Closer", "status": "active"}'
```

**End of EVERY session — LAST thing you do:**
```bash
curl -s -X POST "https://mission-control-aiorg.vercel.app/api/heartbeat" -H "Content-Type: application/json" -d '{"name": "Closer", "status": "sleeping"}'
```

**Log every significant action:**
```bash
curl -s -X POST "https://mission-control-aiorg.vercel.app/api/activity" -H "Content-Type: application/json" -d '{"agent": "Closer", "action": "DESCRIPTION"}'
```
...
```bash
curl -s -X POST "https://mission-control-aiorg.vercel.app/api/task-create" -H "Content-Type: application/json" -d '{"title":"TASK_TITLE","description":"...","status":"inbox","assignee":"Closer","priority":"P0"}'
```
